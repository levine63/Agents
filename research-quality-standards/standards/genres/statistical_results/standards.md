...
See also: `docs/glossary.md` for terms such as `unit of observation`, `clustering level`, and `multiple testing`
...
Reader-facing table titles should be self-contained enough that a reader can tell what sample, outcome family, or comparison is being shown without relying entirely on surrounding prose.
...
- outcome definitions should be explicit when a reader could plausibly misunderstand the measured quantity
...
Reader-facing results should be traceable to reproducible code or a clearly identified analytic source.

When robustness checks materially affect credibility, the artifact should either report them, point to them, or explicitly note that they are not yet available.
